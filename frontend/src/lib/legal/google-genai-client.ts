...
/**
 * Official Google GenAI SDK client for legal analysis
...
export class LegalGoogleGenAIClient {
  ...
  constructor(
    ...
    if (!key) {
      throw new Error('Google GenAI API key is required. Set GEMINI_API_KEY or GOOGLE_API_KEY environment variable.');
    }
    ...
    console.log(`Initialized Google GenAI client with model: ${this.modelName}`);
  }
  ...
  async generateContent(
    ...
    try {
      // Build the request parameters following the SDK documentation
      const requestParams = {
        ...
        config: {
          // System instruction goes HERE, not in contents
      ...
      const response = await this.client.models.generateContent({
        ...
        config: {
          ...
          maxOutputTokens: requestParams.config.maxOutputTokens,
          candidateCount: requestParams.config.candidateCount,
          safetySettings: requestParams.config.safetySettings as SafetySetting[]
        }
      });
      return response;
    } catch (error) {
...
export function getLegalGoogleGenAIClient(): LegalGoogleGenAIClient {
  if (!legalGoogleGenAIClient) {
    ...
    legalGoogleGenAIClient = new LegalGoogleGenAIClient(apiKey, modelName, apiVersion);
  }
